...
class BookController {
    ...
    handleMultipartData(req, res, async (err) => {
      ...
      try {
        ...
        if (isExist) {
          ...
          return next(
            ErrorHandlerService.validationError("ID đã tồn tại")
          );
        }
  ...
  async updateBook(req, res, next) {
    handleMultipartData(req, res, async (err) => {
      ...
      try {
        ...
        return res.status(201).json({ msg: "Cập nhật sách thành công" });
      } catch (error) {
        next(error);
      }
    });
  ...
  async deleteBook(req, res, next) {
    ...
    try {
      ...
      await document.save();
      return res.status(204).json({ message: "Xóa sách thành công" });
    } catch (error) {
      next(error);
    }
  }
  ...
  async exportBooks(req, res, next) {
    try {
      const data = await BookModel.find({ isDeleted: false })
        ...
        .populate("almirah");
      if (data.length === 0) {
        return next(ErrorHandlerService.notFound("Xóa sách thành công"));
      }
