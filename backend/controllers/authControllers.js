...
class AuthController {
  async login(req, res, next) {
    ...
    try {
      ...
      if (!user) {
        return next(
          ErrorHandlerService.wrongCredentials("email không tồn tại")
        );
      }
    } catch (error) {
      next(error);
    ...
    if (!isMatch) {
      return next(ErrorHandlerService.wrongCredentials("Sai mật khẩu"));
    }
  ...
  async changePassword(req, res, next) {
    ...
    try {
      ...
      if (!isMatch) {
        return next(
          ErrorHandlerService.wrongCredentials("Mật khẩu hiện tại không đúng!")
        );
      }
  ...
  async forgetPassword(req, res, next) {
    ...
    try {
      ...
      if (!user) {
        return next(ErrorHandlerService.notFound("Email không tồn tại"));
      }
  ...
  async resetPassword(req, res, next) {
    ...
    try {
      ...
      if (!user) {
        return next(ErrorHandlerService.badRequest("Không tìm thấy người dùng"));
      }
  ...
  async updateProfileImage(req, res, next) {
    ...
    if (!avatar) {
      return next(ErrorHandlerService.validationError("Yêu cầu ảnh đại diện !"));
    }
    try {
      // Image Base64
