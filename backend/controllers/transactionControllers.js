...
class TransactionController {
    ...
    try {
      ...
      if (book.status === "Reserved") {
        ...
        if (user.email === reservedBook?.user?.email) {
          ...
          return next(
            ErrorHandlerService.badRequest("Sách đã được đặt bởi người khác!")
          );
        }
      }
      ...
      book.status = "Issued";
      await book.save();
      return res.status(200).json({ msg: "Mượn sách thành công !" });
    } catch (error) {
      next(error);
    }
  }
  ...
  async userInfo(req, res, next) {
    ...
    try {
      ...
      if (!user) {
        return next(ErrorHandlerService.notFound("Không tìm thấy người dùng"));
      }
      ...
      if (user.role in maxBooksAllowed) {
        ...
        return next(
          ErrorHandlerService.forbidden("Không cho phép mượn sách")
        );
      }
  ...
  async bookInfo(req, res, next) {
    ...
    try {
      ...
      if (!book) {
        return next(ErrorHandlerService.notFound("Không tìm thấy sách"));
      }
  ...
  async returnBook(req, res, next) {
    ...
    if (!transactionID) {
      return next(
        ErrorHandlerService.validationError("Giao dịch đã được yêu cầu.")
      );
    }
    try {
      ...
      if (!transaction) {
        return next(ErrorHandlerService.notFound("Không tìm thấy giao dịch"));
      }
      ...
      return res.status(200).json({ msg: "Trả sách thành công !" });
    } catch (error) {
      return next(error);
    }
  }
  ...
  async payFine(req, res, next) {
    ...
    try {
      ...
      res.status(200).json({ msg: "Trả phí thành công." });
    } catch (error) {
      next(error);
    }
  }
  ...
  async handleRenewRequest(req, res, next) {
    ...
    try {
      ...
      //     subject: "Renewal Request Status",
      ...
      //   return next(new Error("Failed to send email notification."));
      // }
  
      return res.status(200).json({ message: "Xử lý thành công!" });
    } catch (error) {
  ...
  async reservedBook(req, res, next) {
    ...
    const { ISBN } = req.body;
    if (!ISBN) {
      return next(ErrorHandlerService.validationError("Yêu cầu ID."));
    }
    try {
      ...
      if (limit === undefined) {
        return next(
          ErrorHandlerService.forbidden("Không cho phép đặt sách")
        );
      }
      ...
      return res
        .status(201)
        .json({ message: "Đặt sách thành công.", book });
    } catch (error) {
      next(error);
    }
  }
  ...
  async unReservedBook(req, res, next) {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "Hủy đặt sách thành công!", reservedBooks });
    } catch (error) {}
  }
  ...
  async renewRequest(req, res, next) {
    ...
    try {
      ...
      if (
        ...
        return next(
          ErrorHandlerService.badRequest(
            `Yêu cầu của bạn đã được gửi ${transaction.renewStatus}`
          )
        );
      }
      ...
      if (currentDate > transaction.dueDate) {
        return next(
          ErrorHandlerService.badRequest(
            "Không thể gia hạn sách khi đã quá hạn"
          )
        );
      }
